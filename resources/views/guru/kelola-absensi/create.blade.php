...
@section('dashboard-title', 'Tambah Absensi')
...
@section('content')
    <div class="container mx-auto px-4 py-6">
        <div class="mb-6 flex items-center justify-between">
            <div>
                <h2 class="text-2xl font-bold text-gray-800">Tambah Absensi Siswa</h2>
                <p class="text-gray-600 mt-2">Input data kehadiran siswa</p>
            </div>
        ...
        <div class="bg-white rounded-lg shadow-md p-6">
            <form action="{{ route('guru.kelola-absensi.store') }}" method="POST">
                @csrf
                ...
                <div class="mb-6">
                    ...
                    <textarea name="keterangan" id="keterangan" rows="4" placeholder="Tambahkan keterangan jika diperlukan..."
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent @error('keterangan') border-red-500 @enderror">{{ old('keterangan') }}</textarea>
                    @error('keterangan')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    @enderror
                </div>
