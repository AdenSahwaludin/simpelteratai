...
@section('dashboard-title', 'Tambah Pengumuman')
            ...
            <form action="{{ route('admin.pengumuman.store') }}" method="POST">
                ...
                <div class="mb-6">
                    <label class="flex items-center">
                        <input type="checkbox" name="publikasi" value="1" {{ old('publikasi', true) ? 'checked' : '' }}
                            class="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500">
                        <span class="ml-2 text-sm font-medium text-gray-700">Publikasikan (Tampilkan ke semua role)</span>
                    </label>
                    <p class="text-xs text-gray-500 mt-1">Jika tidak dicentang, pengumuman hanya terlihat oleh admin.</p>
                    @error('publikasi')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
