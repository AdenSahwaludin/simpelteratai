...
@section('dashboard-title', 'Tambah Jadwal Harian')
...
@section('content')
    <div class="p-6">
        <div class="mb-6 flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-gray-800">Tambah Jadwal Harian</h1>
                <p class="text-gray-600 mt-2">Buat jadwal kegiatan harian untuk anak TK</p>
            </div>
        ...
        <div class="bg-white rounded-lg shadow-md p-6">
            <form action="{{ route('admin.jadwal-harian.store') }}" method="POST">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div>
                        ...
                        <input type="time" name="waktu_mulai" id="waktu_mulai" required value="{{ old('waktu_mulai') }}"
                ...
                <div class="mb-6">
                    ...
                    <textarea name="kegiatan" id="kegiatan" rows="3" required placeholder="Contoh: Doa & Menyanyi"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 @error('kegiatan') border-red-500 @enderror">{{ old('kegiatan') }}</textarea>
                    @error('kegiatan')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-6">
                    ...
                    <textarea name="catatan" id="catatan" rows="3"
                        placeholder="Contoh: Anak boleh membawa boneka hewan peliharaan ke sekolahan."
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 @error('catatan') border-red-500 @enderror">{{ old('catatan') }}</textarea>
                    @error('catatan')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    @enderror
                </div>
