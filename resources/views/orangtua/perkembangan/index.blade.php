        ...
        <div class="bg-white rounded-lg shadow-md p-4 mb-6">
            <form action="{{ route('orangtua.perkembangan.index') }}" method="GET"
                ...
                <div>
                    ...
                    <select name="mapel_id" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent">
                        <option value="">Semua Mata Pelajaran</option>
                        @foreach ($mapelList as $mapel)
                            <option value="{{ $mapel->id_mata_pelajaran }}" {{ $mapelId == $mapel->id_mata_pelajaran ? 'selected' : '' }}>
                                {{ $mapel->nama_mapel }}
                            </option>
                        @endforeach
                    </select>
