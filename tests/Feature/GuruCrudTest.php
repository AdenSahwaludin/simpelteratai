...
test('admin can view guru index page', function () {
    $response = $this->actingAs($this->admin, 'admin')->get(route('guru.index'));
...
test('admin can view create guru page', function () {
    $response = $this->actingAs($this->admin, 'admin')->get(route('guru.create'));

    $response->assertStatus(200);
    $response->assertViewIs('guru.create');
});
...
test('admin can create a new guru', function () {
    ...
    $response = $this->actingAs($this->admin, 'admin')->post(route('guru.store'), $guruData);
...
test('admin can view guru details', function () {
    ...
    $response = $this->actingAs($this->admin, 'admin')->get(route('guru.show', $guru->id_guru));

    $response->assertStatus(200);
    $response->assertViewIs('guru.show');
...
test('admin can view edit guru page', function () {
    ...
    $response = $this->actingAs($this->admin, 'admin')->get(route('guru.edit', $guru->id_guru));

    $response->assertStatus(200);
    $response->assertViewIs('guru.edit');
    $response->assertViewHas('guru', $guru);
});
...
test('admin can update guru', function () {
    ...
    $response = $this->actingAs($this->admin, 'admin')
        ->put(route('guru.update', $guru->id_guru), $updateData);
...
test('admin can delete guru', function () {
    ...
    $response = $this->actingAs($this->admin, 'admin')
        ->delete(route('guru.destroy', $guru->id_guru));
...
test('guru validation requires all fields', function () {
    $response = $this->actingAs($this->admin, 'admin')->post(route('guru.store'), []);
